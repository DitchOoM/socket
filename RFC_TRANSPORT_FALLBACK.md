...
**Status:** v2 implemented (`com.ditchoom.socket.transport`): the MVP surface (`FallbackTransport`, `FallbackPolicy`, `CapabilityCache`, `NetworkId`, `TransportSet`/`defaultTransportChain`) **plus** §5 staggered family racing (opt-in `stagger`, lanes drawn by `Transport.family`), the wired two-scope learning cache (`InMemoryCapabilityCache` is the production default, thread-safe), the §6 per-network write (whole-UDP-family-timeout + TCP-success contrast in one connect), and the `NetworkMonitor.networkId` producer (Apple `NWPathMonitor` interface type+index, Android `ConnectivityManager` transports+`networkHandle`, browser `navigator.connection.type`; desktop JVM / Linux native / Node stay `Unidentified` — no reliable link-kind API). Remaining "later, only if measured": Alt-Svc, N-strikes soft-demote.
...
- ~~Is `networkId` cheaply observable on every platform?~~ **Resolved by making absence first-class**: `NetworkId` is a sealed model where "can't identify the network" is the explicit `Unidentified` case that simply disables the per-network scope (no entry recorded or read), and browsers get the coarse `KindOnly` case (`navigator.connection.type`) — still enough for the decisive Wi-Fi↔cellular transition. The reactive `NetworkMonitor` is the population source — **wired in v2**: `NetworkMonitor.networkId: StateFlow<NetworkId>` (default `Unidentified`; real producers on Apple/Android/browser-JS) feeds `FallbackTransport(networkId = { monitor.networkId.value })`, which stamps it onto `TransportConfig` per connect.
