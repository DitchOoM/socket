...
**Design forks LOCKED (2026-06-15, user-approved):**
- **Module shape:** new `:socket-webtransport` depends on `:socket-http3` **only in jvm/android/native** source sets; the **browser (wasmJs/js)** source set pulls *neither* socket-http3 nor socket-quic (wraps the native `WebTransport` object directly — §9 honesty-by-construction). Streams are the already-neutral buffer types from Phase 3a (`ByteStream`/`ByteSink`/`ByteSource` + `Resettable`) — no new stream abstraction.
- **Fork 1 (session type boundary) → A: neutral interface + adapters.** Define a platform-neutral `interface WebTransportSession` in socket-webtransport commonMain; native actual wraps socket-http3's concrete class in a thin delegating adapter (streams pass straight through — zero per-stream wrapping), browser actual wraps the JS object. Rejected B (lift http3 class to commonMain → would force a QUIC-typed field into the browser build = an impossible state) and C (expect/actual typealias → still needs the sealed layer anyway + experimental-class fragility). Native-only power lives on the *separate* sealed `WebTransportSupport.Multiplexed`, reachable only by smart-cast.
- **Fork 2 (native held lifetime) → 1 now, evolve to 2.** `connect(url)` returns a HELD session whose private session-owned `CoroutineScope` runs `withHttp3Connection { connectWebTransport(...); awaitCancellation }`; `session.close()` cancels the scope (structured-concurrency teardown — can't leave a half-state). This keeps socket-quic's scope-only `QuicScope` invariant ("no close(); the block IS the lifecycle") UNTOUCHED. Evolve to option 2 (`connect` = `Multiplexed.openSession` underneath, one unified held-connection lifetime) when `Multiplexed` lands. **Rejected option 3** (new unscoped `Http3Connection.open()` primitive) — reintroduces the TOCTOU-representable bad states the scope model deletes.
- **DONE bar:** `Multiplexed` (native-only, many sessions over one held H3 conn) MUST be implemented + validated before Phase 4 is called done — not just `connect(url)`. Build the API shape (interface + sealed `WebTransportSupport` + both expect/actual stubs) so it **compiles once**, stop for human review, THEN fan out the impls. socket-websocket comes AFTER socket-webtransport compiles.
