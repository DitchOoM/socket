 ...
 * rootless netns harness cannot reach: **Wi-Fi**. [LinuxNetworkMonitor.classifyLinkKind] maps an
 * interface to `NetworkKind.Wifi` when `/sys/class/net/<iface>/phy80211` (or the legacy `wireless`)
 * entry exists — which `dummy`/`tun`/`tuntap` devices never have. A real (or simulated) 802.11 device
 * is required, so the harness loads the `mac80211_hwsim` kernel module (privileged) to get a `wlanN`
 * device with a real `phy80211`, then runs this against its live `/sys` view.
 *
 ...
 *
 * Self-skips unless `NETMON_WIFI_IFACE` is set, so a plain host `:network-monitor:linuxX64Test` run is
