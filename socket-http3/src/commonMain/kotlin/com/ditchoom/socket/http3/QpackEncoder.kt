...
class QpackEncoder(
     ...
     */
    fun processDecoderInstruction(instruction: QpackDecoderInstruction) {
        when (instruction) {
            ...
            is QpackDecoderInstruction.SectionAck -> {
                val ric =
                    outstandingSectionRic[instruction.streamId]?.removeFirstOrNull()
                        ?: throw decoderStreamError("Section Acknowledgment for stream ${instruction.streamId} with no outstanding section")
