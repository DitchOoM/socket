...
class QpackFieldSectionPrefixTests {
    private fun roundTrip(
        ...
        buf.resetForRead()
        return QpackFieldSectionPrefix.decode(buf, maxEntries, InsertCount(totalInserts))
    }

    @Test
    fun zeroRequiredInsertCountAndBase() {
        // The static-only prefix: RIC 0, Base 0 — unaffected by MaxEntries / totalInserts.
        assertEquals(QpackPrefix(InsertCount(0), 0), roundTrip(0, 0, maxEntries = 31, totalInserts = 0))
    }

    @Test
    fun baseEqualToRequiredInsertCount() {
        assertEquals(QpackPrefix(InsertCount(3), 3), roundTrip(3, 3, maxEntries = 31, totalInserts = 3))
    }

    @Test
    fun baseGreaterThanRic_sign0() {
        // Base > RIC: S=0, DeltaBase = Base - RIC.
        assertEquals(QpackPrefix(InsertCount(3), 7), roundTrip(3, 7, maxEntries = 31, totalInserts = 7))
    }

    @Test
    fun baseLessThanRic_sign1_postBase() {
        // Base < RIC: S=1, DeltaBase = RIC - Base - 1 (post-Base references follow).
        assertEquals(QpackPrefix(InsertCount(5), 2), roundTrip(5, 2, maxEntries = 31, totalInserts = 5))
    }
    ...
    fun requiredInsertCountWrapsAroundMaxEntries() {
        ...
        assertEquals(QpackPrefix(InsertCount(25), 25), roundTrip(25, 25, maxEntries = 10, totalInserts = 25))
    }
    ...
    fun decodeRecoversRicWhenDecoderHasInsertedMoreSince() {
        ...
        assertEquals(InsertCount(8), roundTrip(8, 8, maxEntries = 31, totalInserts = 12).requiredInsertCount)
    }
    ...
    fun decodeRejectsEncodedInsertCountAboveFullRange() {
        ...
        buf.resetForRead()
        assertFailsWith<DecodeException> { QpackFieldSectionPrefix.decode(buf, maxEntries = 10, totalInserts = InsertCount(0)) }
    }
    ...
    fun decodeRejectsDynamicReferenceWhenTableDisabled() {
        ...
        buf.resetForRead()
        assertFailsWith<DecodeException> { QpackFieldSectionPrefix.decode(buf, maxEntries = 0, totalInserts = InsertCount(0)) }
    }
}
