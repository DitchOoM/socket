...
| Apple | ~~Axis 2 (destructive), Axis 3 (`TimeoutCancellationException`)~~ **conformant (Phase 4b landed, macOS-CI-validated)** | `NWSocketWrapper.readRaw` no longer wires `closeInternal()` into a per-read cancellation handler. Network.framework has no per-receive cancel, so the native `nw_helper_tcp_receive` is left outstanding and its one-shot completion captured in a socket-held `CompletableDeferred` (single-flight, §3.2); the caller `withTimeout`s only the `await`. On timeout the deferred is orphaned for the next `read()` to re-await (Axis 2) and a `SocketTimeoutException` is thrown (Axis 3); `closeInternal()` now runs only on genuine EOF / error / `close()`. |
...
4. **Fix destructiveness (JVM NIO2 default + Apple).** The hardest slice — the orphaned-read / non-destructive receive work. Turns Axis 2 green. Requires the §3.2 buffer-lifetime discipline.
   ...
   - **4b — Apple. ✅ LANDED (macOS-CI-validated).** `NWSocketWrapper.readRaw` no longer attaches `closeInternal()` to a per-read cancellation handler. Since Network.framework has no per-receive cancel (§9), the native `nw_helper_tcp_receive` is left outstanding and its one-shot completion captured in a socket-held `CompletableDeferred` (single-flight); the caller `withTimeout`s only the `await`, orphaning the receive on timeout for the next `read()` to re-await. `closeInternal()` runs only on genuine EOF / error / `close()`. Authored hand-indented on Linux (appleMain neither compiles nor lints there); verified on the macOS CI lane.
5. **(Optional) VT strategy.** Only after 1–4. Adds `IoConcurrency` + the allocation seam (§7).

## 9. Open questions

- **Non-destructive on Apple — resolved (Phase 4b).** Network.framework has **no** per-receive cancel (unlike io_uring's single-SQE cancel), so the fix is exactly the orphaned-coroutine pattern anticipated here: the native `nw_helper_tcp_receive` is left outstanding and its one-shot completion captured in a socket-held `CompletableDeferred`, re-awaited by the next `read()`. Buffer lifetime is trivially safe (§3.2) because the NSData buffer is materialized only *inside* the completion callback — a timed-out caller can never free a buffer a live receive is still filling. Landed hand-indented; macOS-CI-validated.
