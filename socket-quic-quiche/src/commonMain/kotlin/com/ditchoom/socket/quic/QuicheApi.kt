...
interface QuicheApi {
     ...
     *
     * Diagnostics only: [QuicheDriver] calls this once per connection — when its capture names a
     * [com.ditchoom.socket.quic.trace.QlogTarget.File] or `QUIC_QLOG_DIR` is set — on the driver
     * coroutine right after the connection is created (quiche is single-threaded — it must not be called
     * concurrently with the driver loop). One `.sqlog` file per connection: quiche opens [path] with
     * `create_new`, so a path already on disk is refused (`false`) rather than appended to or overwritten.
     * The interface default is a no-op returning `false`, so test doubles need not implement it; every
     * real backend (FFM, JNI/Android, cinterop) overrides it. Strings (not native addresses) so the JNI
     * backend can `GetStringUTFChars`.
     */
    fun connSetQlogPath(
        conn: QuicheConn,
        path: String,
