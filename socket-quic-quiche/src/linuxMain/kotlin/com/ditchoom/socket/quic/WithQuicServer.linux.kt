...
/**
 * Build + bind a Linux quiche-backed server over the `:socket-udp` io_uring datagram channel (Phase 6
 * adapter-first cutover), returning it ready to accept. The returned [SharedQuicheServer] owns its
 * teardown via [SharedQuicheServer.close]; the `onClose` lambda cancels the per-call parent scope.
 * Shared by [QuicheEngine.bind]; the [withQuicServer] wrapper runs the block and calls `close()`. The
