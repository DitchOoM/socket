...
/**
 * Build + bind a Linux quiche-backed [LinuxQuicServer] over the `:socket-udp` io_uring datagram channel
 * (Phase 6 adapter-first cutover), returning it ready to accept. The returned server owns its teardown
 * via [LinuxQuicServer.close]; the `onClose` lambda cancels the per-call parent scope. Shared by
...
internal suspend fun buildLinuxQuicServer(
    ...
    try {
        ...
        // buffer. One channel serves every accepted connection; per-connection egress is a thin
...
/**
 * Linux QUIC server over the `:socket-udp` [DatagramChannel]. Structurally mirrors `JvmQuicServer`:
 * a dedicated reader coroutine confines the channel's `receive()` and feeds a central loop that routes
 * each datagram by DCID to its [QuicheDriver] (or accepts a new connection). All connection-lifecycle
 * bookkeeping (routing table, live-driver ledger, per-source recv_info cache, close sweep) lives in the
 * shared [ServerConnectionRegistry] — the #179 recv_info-UAF invariants are fix-once.
 */
private class LinuxQuicServer(
     ...
     * recycles it back. Hoisted to a ctor param (default: a fresh pool) so [buildLinuxQuicServer] injects
    ...
    override suspend fun close() {
        if (closed) return
        closed = true
        // Closing the :socket-udp channel makes any in-flight io_uring recvmsg return promptly, so the
        // reader's receive() returns Closed, the loop exits, and receiveJob completes.
        channel.close()
        receiveJob.join()
