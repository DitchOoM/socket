...
internal suspend fun buildJvmQuicConnection(
    ...
    try {
        ...
        //
        // The path factory is built here, ahead of the channel, because the primary path *is* a path:
        // it is opened by the same [UdpSocketChannelFactory.openConnectedChannel] every migration path
        // goes through, so its source address is named rather than left to a wildcard bind (#519). It is
        // also the factory the driver's migration wiring takes at step 7, so one object decides how every
        // socket to this peer is bound and an unnamed primary bind has no way to be written here.
        //
        // This used to be a bare `UdpSocket.connect(peer)` — `bind([::], 0)` then `connect` — behind an
        // 8-attempt BindException retry whose KDoc called the collision a race between concurrent binds.
        // It is not a race: it is single-threaded, deterministic, and it is #434's mechanism, which the
        // retry lowered to `(k / 16384)^8` instead of removing. Measured on macOS/JDK 21 with 3000
        // sockets held to one peer: 264 EADDRINUSE at `connect` unnamed, 0 named. A source the route
        // probe cannot name now throws UnresolvedRouteSourceException rather than falling back to the
        // unnamed bind, exactly as `openPath` does — and ProbeRefused still carries the platform's own
        // exception as its cause, so the caller loses nothing by the wrap.
        ...
        val channel = pathFactory.openConnectedChannel(localHost = null, localPort = 0)
