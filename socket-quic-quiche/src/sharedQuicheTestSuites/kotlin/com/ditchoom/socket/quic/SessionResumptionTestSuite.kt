 ...
 * session ticket, closes, and a second connection offers that ticket with a [QuicEarlyData.Replayable]
 * block. What the handshake did is read from both ends — the client's [QuicScope.resumption] and the
...
abstract class SessionResumptionTestSuite {
    ...
    private val clientOptions =
        QuicOptions(
            alpnProtocols = listOf("resume-test"),
            verifyPeer = false,
                    ...
                    try {
                        ...
                        val outcome =
                            withQuicConnection(
                                "127.0.0.1",
                                port,
                                clientOptions.copy(resumption = QuicResumption.Offer(ticket)),
                                timeout = 10.seconds,
                            ) {
                        ...
                        assertEquals(
                            ...
                            outcome,
                            "an Offer with QuicEarlyData.None resumes without enabling 0-RTT on the client. Server saw $second",
                        )
                        assertEquals(
        ...
        runQuicTest(timeout = 60.seconds) {
            wrapTestBody {
                withQuicServer(port = 0, tlsConfig = testTlsConfig(), quicOptions = acceptsEarlyData) {
                    val seen = serveEach(this)
                    try {
                        val garbage = QuicSessionTicket.restore(text("not a session ticket"))
    ...
    /** Reconnect offering [ticket], with the payload written in a [QuicEarlyData.Replayable] block. */
    private suspend fun earlyReconnect(
        ...
        val options =
            clientOptions.copy(
                resumption =
                    QuicResumption.Offer(
                        ticket,
                        QuicEarlyData.Replayable {
