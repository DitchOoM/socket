...
/**
 * Linux K/Native member of [RetiredCidInFlightPacketTestSuite].
 *
 * Linux is where the field failures were captured (the public echo server the device rig drives is a
 * Linux host), so this member checks the fix on the same native the measurements came from.
 *
 ...
 */
class LinuxRetiredCidInFlightPacketTests : RetiredCidInFlightPacketTestSuite() {
    ...
    internal override suspend fun buildServer(
        ...
        options: QuicOptions,
        api: QuicheApi,
    ): SharedQuicheServer = buildLinuxQuicServer(binding, tlsConfig, options, api = api)
}
