...
/**
 * linuxX64 mirror of [JvmCallerClockSimTests]: Option 2 of P7 (RFC_UNIFIED_NETWORK_TEST_HARNESS.md §6.1,
 * remaining item b) on the K/N cinterop backend. Proves the **production seam** — `SimClock.quicheTime()`
 * → [CallerClockQuicheApi] decorator → `quiche_set_virtual_time_nanos` — drives real libquiche's internal
 * clock, not just the raw FFI that [LinuxCallerClockTests] already covers.
 *
 * A [SimClock] on a standalone [TestCoroutineScheduler] plays the Tier-A virtual clock; the real
 * [CinteropQuicheApi] is wrapped exactly as [QuicheDriver] wraps it for a `DriverTime.Virtual` clock, so
 * every steady-state `connTimeout`/`connOnTimeout` pins libquiche's per-thread clock first. The assertion
 * is bit-exactness of a *structural* (clock-driven, RNG-independent) trace across two independent runs,
 * with the timer strictly shrinking under advancing virtual time and a final idle-timeout on the virtual
 * clock alone. See [JvmCallerClockSimTests] for the full rationale.
 */
class LinuxCallerClockSimTests {
