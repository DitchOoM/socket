 ...
 *
 * Apple's server is the same Cloudflare-quiche server as Linux/JVM, on a dual-stack POSIX UDP socket,
 * doing the same per-source `recv_info` + `sendInfo.to` egress routing, so this member asserts the real
 * RFC 9000 §9.3 behaviour with no way to opt out of it.
 *
