...
/**
 * Apple (Kotlin/Native) [QuicheApi] implementation backed by K/N cinterop into the macOS libquiche.a.
 *
 * A near-verbatim copy of the linux `CinteropQuicheApi`: every `quiche_*` call is bound from the same
 * `Quiche.def` cinterop, so the entire surface is identical except the BSD/Darwin sockaddr decode at
 * the bottom (see that section).
 *
 * ⚠️ **Every FFI fix here is owed to the linux copy too, and vice versa.** `CinteropQuicheApiDriftGuardTest`
 * fails CI on the commit that lets the two diverge.
 *
...
internal object CinteropQuicheApi : QuicheApi {
    ...
    // BSD/Darwin layout differs from Linux: sa_len (uint8) at byte 0, sa_family (uint8) at byte 1
    // (Linux has no sa_len and stores sa_family as a uint16 LE at byte 0). The port (BE uint16 at
    // offset 2), sin_addr (offset 4) and sin6_addr (offset 8) sit at the same offsets on both, so
    // only the family read differs. Darwin AF_INET6 = 30 (Linux = 10) — use the posix constants so
    // the right value is compared per platform.
    ...
    private const val QUICHE_ERR_STREAM_RESET = -16
}
