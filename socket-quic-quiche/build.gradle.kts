...
// --- Build stamp: the commit a walk binary was built from ---
//
// Stamped into what a walk runs — the quic-echo jar (as a resource) and the Android probe (as a
// constant) — so every START line names the build it came from. Runs on every build, because the
// answer moves with every commit and every edit, but rewrites a file only when the stamp changed, so a
// build whose stamp did not move recompiles nothing. An empty stamp means git had no answer; the
// binary reports that as `build=unknown(NotStamped)` rather than leaving the field out.
val buildStampDir = layout.buildDirectory.dir("generated/buildStamp")
val generateBuildStamp =
    tasks.register("generateBuildStamp") {
        group = "build"
        description = "Stamp the git commit into the quic-echo jar and the Android probe"
        ...
        doLast {
            ...
            writeIfChanged(dir.resolve("resources/build-revision.txt"), stamp + "\n")
            writeIfChanged(
                dir.resolve("kotlin/com/ditchoom/socket/quic/ProbeBuildStamp.kt"),
                "// Generated by :socket-quic-quiche:generateBuildStamp — DO NOT EDIT.\n" +
...
afterEvaluate {
    ...
    tasks.register<Jar>("quicEchoJar") {
        ...
        // The commit this jar was built from, read by QuicEchoTestServer for its START line.
        from(generateBuildStamp.map { buildStampDir.get().dir("resources") })
...
kotlin {
    ...
    sourceSets {
        ...
        val androidInstrumentedTest by getting {
            kotlin.srcDir("src/sharedJvmTestProtocol/kotlin")
            // PROBE_BUILD_STAMP, for DeviceHandoffProbe's START line.
            kotlin.srcDir(generateBuildStamp.map { buildStampDir.get().dir("kotlin") })
