...
#
# The target comes from SERVER_HOST in common.sh: one host, or a COMMA-separated rotation the probe
# walks one target per connection attempt, so one phone covers both address families on one route.
...
# probeHost is one host or a COMMA-separated rotation (see common.sh). Quoted twice on purpose: once
