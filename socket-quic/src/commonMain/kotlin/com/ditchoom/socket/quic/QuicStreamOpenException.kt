...
/**
 * `openStream()` could not create the stream.
 *
 * Opening a stream materialises it with quiche immediately rather than only reserving an id locally,
 * so a refusal is known at `openStream()` and reported there. The dominant case is the
 ...
 * limit.
 *
 * This exists because discarding that refusal would hand back a slot quiche had refused to create,
 * and the next read on it would answer `INVALID_STREAM_STATE` — "the stream is in a bad state" — for
 * a stream that had never been created.
...
class QuicStreamOpenException(
     ...
     * seen, so those are reported by the first real write.
     */
