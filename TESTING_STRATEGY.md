...
**Phase 2 — TLS cert matrix (L6).** Add the `tls` container, `gen-certs.sh`, the CA-trust CI steps (§3c/§3d). Migrate `TlsErrorTests`, `NetworkIntegrationTests.tls_*`, `SniStrictHostsTest`, `LinuxTlsTests` into the `TlsConformanceTests` suite. Move `SniStrictHostsTest` and the valid-cert test to `commonTest`. Delete the public-host originals once the harness versions pass on JVM + Linux + Apple.
