...
# resident afterwards by listing the process. Needs the phone paired (wireless is fine).
set -euo pipefail
. "$(dirname "$0")/common.sh"
...
xcodebuild -project QuicProbe.xcodeproj -scheme QuicProbe -configuration Debug -destination "id=$DEVICE" -derivedDataPath "$DD" -allowProvisioningUpdates build 2>&1 | grep -E "error:|BUILD SUCCEEDED|BUILD FAILED" | head -5
