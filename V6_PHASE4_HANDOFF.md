...
- New, all **gated behind `-PwtBrowserInterop`** so default builds/CI never require Chrome (verified: without the flag the gated source isn't compiled and `generateBrowserInteropConfig` isn't registered): `socket-webtransport/src/jvmTest/.../BrowserInteropServer.kt` (gated `-Dwt.interop.server=true` harness — ephemeral port, pinned cert, echoes, writes `build/wt-interop/config.properties`), `src/browserInterop/.../BrowserWebTransportInteropTest.kt` (a **shared** srcDir compiled into BOTH `jsTest` and `wasmJsTest` — platform-neutral, `allocateNative`), `scripts/browser-interop.sh` (orchestrates server↔`jsBrowserTest`+`wasmJsBrowserTest`), and build wiring (`useChromeHeadless` for js+wasmJs, generated `BrowserInteropConfig.kt`, js/wasmJs test srcDir, ktlint exclude of the generated file). **2026-06-24 update:** the reset→neutral-exception parity test now also runs on **wasmJs** in Chrome (3/3); fixed a wasmJs-only bug where coroutines' `Promise.await()` discarded the `WebTransportError` rejection value (added `awaitOrThrowRejection` preserving the JS reason).
