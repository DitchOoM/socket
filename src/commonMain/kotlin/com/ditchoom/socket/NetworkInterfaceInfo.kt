 ...
 *
 * [index] is the OS interface index — the same numeric handle a
 * [NetworkId.Link][com.ditchoom.socket.transport.NetworkId.Link] carries, so a gathered candidate can
 * be tied back to the network the [NetworkMonitor] reports as primary. [kind] is best-effort:
 * [NetworkKind.Other] on the raw-scan platforms (desktop JVM / Android / Node — a bare interface scan
 * cannot tell Wi-Fi from Ethernet), semantically classified on Linux native (from `/sys/class/net`).
 * [addresses] are numeric IPv4/IPv6 literals (never DNS), an IPv6 link-local possibly carrying its
 * `%zone` — the diagnostic-friendly form ICE host-candidate gathering consumes.
 */
