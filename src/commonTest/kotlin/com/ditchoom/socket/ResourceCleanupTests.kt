...
class ResourceCleanupTests {
    ...
    @Test
    fun socketClosedAfterUseBlock() = runTestNoTimeSkipping {
    ...
    @Test
    fun socketClosedOnException() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
    ...
    @Test
    fun serverCleanupOnException() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
    ...
    @Test
    fun repeatedOpenClose() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
    ...
    @Test
    fun coroutineCancellationCleansUpSocket() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
        ...
        val clientJob = launch(Dispatchers.Default) {
            val client = ClientSocket.allocate()
            clientRef = client
            client.open(server.port(), timeout = 5.seconds, hostname = "127.0.0.1")
    ...
    @Test
    fun serverFlowCancellation() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
        var collectCount = 0
    ...
    @Test
    fun timeoutDoesNotLeakResources() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
    ...
    @Test
    fun serverAcceptsContinuesAfterClientError() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
        ...
        val serverJob = launch(Dispatchers.Default) {
            serverFlow.collect { client ->
                try {
                    ...
                    val data = withTimeout(200.milliseconds) {
