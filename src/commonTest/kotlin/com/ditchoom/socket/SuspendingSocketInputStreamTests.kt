...
class SuspendingSocketInputStreamTests {
    @Test
    fun ensureBufferSize_doNotReadAhead_NoCurrentBuffer_NotFragmented() = block {
    ...
    @Test
    fun ensureBufferSize_doNotReadAhead_NoCurrentBuffer_Fragmented() = block {
        val reader = object : Reader {
    ...
    @Test
    fun ensureBufferSize_doNotReadAhead_HasCurrentBuffer_Fragmented() = block {
        val reader = object : Reader {
    ...
    @Test
    fun ensureBufferSize_shouldReadAhead_HasDeferredBuffer_HasCurrentBuffer_Fragmented() = block {
        val reader = object : Reader {
            private var internalCount = 1.toByte()
            override fun isOpen(): Boolean = true
            override suspend fun read(timeout: Duration): ReadBuffer {
    ...
    @Test
    fun ensureBufferSize_nullSize_NoCurrentBuffer() = block {
        val reader = object : Reader {
            private var internalCount = 1.toByte()
            override fun isOpen(): Boolean = true
            override suspend fun read(timeout: Duration): ReadBuffer {
