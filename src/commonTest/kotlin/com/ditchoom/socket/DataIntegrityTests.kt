...
class DataIntegrityTests {
    ...
    @Test
    fun binaryDataTransfer() = runTestNoTimeSkipping {
    ...
    @Test
    fun largeDataTransfer_64KB() = runTestNoTimeSkipping {
        testLargeDataTransfer(64 * 1024) // 64 KB
    }

    @Test
    fun largeDataTransfer_256KB() = runTestNoTimeSkipping {
        testLargeDataTransfer(256 * 1024) // 256 KB
    }

    @Test
    fun largeDataTransfer_1MB() = runTestNoTimeSkipping {
    ...
    private suspend fun testLargeDataTransfer(size: Int) {
        ...
        val serverJob = kotlinx.coroutines.CoroutineScope(Dispatchers.Default).launch {
            serverFlow.collect { client ->
    ...
    @Test
    fun bidirectionalDataTransfer() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
    ...
    @Test
    fun multipleSmallWrites() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
        val dataReceived = Mutex(locked = true)
        ...
        val serverJob = launch(Dispatchers.Default) {
            serverFlow.collect { client ->
                ...
                repeat(messageCount) {
                    try {
                        val chunk = client.readString(timeout = 5.seconds)
                        sb.append(chunk)
        ...
        repeat(messageCount) { i ->
            val msg = "msg$i;"
            expectedData.append(msg)
            client.writeString(msg)
        }
        client.close()

        dataReceived.lock()
        assertEquals(expectedData.toString(), receivedData, "All small messages should be received in order")
    ...
    @Test
    fun partialReadHandling() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
    ...
    @Test
    fun nullBytesInData() = runTestNoTimeSkipping {
        val server = ServerSocket.allocate()
        val serverFlow = server.bind()
        val dataReceived = Mutex(locked = true)
