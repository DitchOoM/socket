...
- [x] ~~**TLS 1.3-only scenario** (`tlsFirstReadReturnsData`) — needs a dedicated nginx block with `ssl_protocols TLSv1.3` on a sixth port (`TLS_TLS13_PORT=14493`, currently reserved/commented out in `harness.env`).~~ (done: Phase 4 — sixth server block on 14493, `tlsHarnessFirstReadReturnsData` re-routed there.)
...
- [ ] **In-repo unit test for the iteration logic — DEFERRED (Phase 4).** Investigated during Phase 4 (dnsmasq + multi-address fixture). `LinuxClientSocket.open()` builds the addrinfo struct inside `memScoped` via cinterop `getaddrinfo` — there's no internal `connect(addrinfoList)` overload, no constructor parameter for a pre-resolved address list, and no `ResolverStrategy` interface to inject. Wiring a dnsmasq container to steer the unit-test resolver requires either root (to edit `/etc/resolv.conf`) or running tests inside a container (defeats the linuxX64 test ergonomics). A purely-synthetic test that stubs `getaddrinfo` via cinterop interception would have low signal/cost ratio for the regression we'd be guarding against. Defer until either (a) `LinuxClientSocket` grows a testable injection point, or (b) Phase 5 ships container-side test execution that can pick up a sidecar DNS server.
- [ ] **IPv6-only host coverage — DEFERRED (Phase 4).** Same blocker as the addrinfo iteration test: no resolver-injection seam. Currently exercised in-passing whenever a test connects via `localhost` on a dual-stack host (gets `::1` AAAA before `127.0.0.1` A). Worth converting to an explicit assertion once the injection seam exists.
