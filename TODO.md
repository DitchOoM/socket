...
- [ ] **CI-only handshake hang on late-suite jvmTest tests.** 9 tests skipped on GH Actions ubuntu-24.04 runners via `assumeTrue(System.getenv("CI") == null)`:
    ...
    All 9 fail at exactly their `withTimeout(10.seconds)` budget. Diagnostic instrumentation (per-step `System.err.println` w/ flush) over 15+ CI cycles shows the QUIC handshake never completes — neither side gets past the initial-packet exchange. Passes locally (Ubuntu 24.04 WSL2, 24 cores) in <130ms across all 9. Reproduction attempted with `-XX:ActiveProcessorCount=1`, `-Xmx512m`, `--workers=2`, `taskset -c 0` — none reproduce. Tests using the same `engine.connect()` + `server.connections {}` pattern (`JvmQuicServerTestSuite.echoSingleStream` 113ms, `QuicLocalServerTests.echoSingleStream` 111ms, `JvmQuicServerTestSuite.serverAcceptsConnection` 8ms) PASS on the same CI run, so it's not the connect pattern per se — the failing tests are all alphabetically late, suggesting cumulative JVM/process state. Adding `serverEngine.close()` in cleanup didn't help. Worth a fresh look with a self-hosted runner, JFR profiling, or `quic-interop-runner` for protocol-level interop coverage.
